  ...
  def test_1_evalate
	...
	def test_wikiparser
	  cs = SpiderMonkey::Context.new
		unless File.exist? "WikiParser.js" 
			p "no WikiParser. test through"
			# wiki parser get from 
			#  http://dev.ishinao.net/WikiParser/javascript/
			#  http://tdiary.ishinao.net/20050403.html#p05
			return;
		end
		open( "WikiParser.js" ){|f|
			cs.eval( f.read );
		}
		cs.eval( <<-JAVASCRIPT );
			function htmlencode(txt){ 
				return txt.replace(/\&/g,'&amp;').replace(/\</g,'&lt;').replace(/>/g,'&gt')
			}
			function doParse(text){
				var parser = new WikiParser();
				parser.options['heading_depth'] = 3;
				parser.options['paragraph_oriented'] = true;
				parser.disablePlugins = {'html': true};
				return parser.parse(text);
			}
		JAVASCRIPT
		html=""
		assert_nothing_thrown {
			html = cs.eval( %! doParse('hoge'); !);
		}
	end
	...
	def test_class_set_property
		...
		assert_raise(NoMethodError){|| cx.eval("x.name2='fuga'") }
